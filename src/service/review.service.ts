...
export async function addReviewReturnAvgRating(userId: string, borrowId: string, rating: string) {
    logger.info("Called addReviewReturnAvgRating");
    const bookId = await BorrowModel.getBookIdByBorrowId(borrowId);
    const newAvgRating = await ReviewModel.addReviewReturnAvgRating(+userId, +bookId, +rating); 
    await BorrowModel.updateIsReviewed(borrowId); // update is_reviewed to true in users_issued_books
    await BookModel.updateBookById(bookId, { rating: +newAvgRating },userId); // update book rating in books
}